...
agilent.twocolors.analysis <- function(pd.file, id.rem.samp = NULL, type.data = "mRNA",
  ...
  rgList <- limma::read.maimages(files=unique(pd[,id_file_names]), source="agilent.median", verbose = verbose)
...
get.sva.batch.effects <- function(comb.data, pd, vars, npc = 10, verbose = T, cmd.ui = T) {
  ...
  #Comment pc.anaylsis.2, it is crashing for singluar column X
...
get.genes <- function(geneids, mart, species="Homo sapiens", type) {
  ...
  if(species=="Homo sapiens")
    gannot <- biomaRt::getGene(id = geneids, type = type, mart = mart)[,c(type, "hgnc_symbol","description","ensembl_gene_id")]
  else {
    gannot <- biomaRt::getGene(id = geneids, type = type, mart = mart)  #[,c(type, "symbol","description","ensembl_gene_id")]
...
plot.heatmpa.genes <- function(expr.dat, col, sf = 3, ze = 0.4, pi = 0.4, o = 1, ccolors) {
  ...
  dat$value <- round(reshape2::melt(attr(dfp,"ifs"))$value, 2)
  dat$labels <- reshape2::melt(dl)$value
  print(dat)
...
merge.array <- function (esets)  {
  eset1 = esets[[1]]
  annot1 = Biobase::annotation(eset1)
  for (i in 2:length(esets)) {
    ...
    Biobase::fData(eset1) = fData
    annot1 = c(annot1, Biobase::annotation(eset2))
  }
  Biobase::annotation(eset1) = unique(annot1)
  return(eset1)
}
...
get_color_palette <- function(iVec, asFactor=FALSE){
		...
		#n <- length(nm)
                #nmPalette <- setNames(sample(color, n), seq(n))
		...
		#color <- grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
		#n <- length(iVec)
                #colorVec <- setNames(sample(color, n), iVec)
...
get_affyBatchObject <- function(fileNames, celDir=NULL, pheno, cdfname){
	print("In get_affyBatchObject function...")
