...
## PRE-PROCESSING
fix.data <- function(pd, id.fn, rgList, GR = TRUE, verbose = TRUE) {
  # merging the two channels
  if(GR)
    data <- cbind(rgList$G, rgList$R)
  else
    data <- cbind(rgList$R, rgList$G)
  rownames(data) <- rgList$genes$ProbeName
  # order the pd file
  pd[,id.fn] <- as.character(pd[,id.fn])
  file.names <- unique(colnames(data))
  #print(file.names)
  id.G <- unlist(sapply(file.names, function(x) {
    if(GR) {
      id <- grep(x, pd[,id.fn])
      return(id[which(pd$dye[id] == "Cy3" | pd$dye[id] == "cy3")])
    }
    else {
      id <- grep(x, pd[,id.fn])
      return(id[which(pd$dye[id] == "Cy5" | pd$dye[id] == "cy5")])
    }
  }))
  id.R <- unlist(sapply(file.names, function(x) {
    if(GR) {
      id <- grep(x, pd[,id.fn])
      return(id[which(pd$dye[id] == "Cy5" | pd$dye[id] == "cy5")])
    }
  ...
  if(length(id.rem) == 0) {
    ...
    if(verbose) {
      cat("Removing indicated samples ..\n")
      print(pd[id.rem,])
    }
    pd <- pd[-id.rem,]
    data <- data[,-id.rem]
  ...
  #if(length(id.slide) > 0)
  #pd[,id.slide] <- as.factor(pd[,id.slide])
  #pd <- pd[,-grep("ample",colnames(pd))]
