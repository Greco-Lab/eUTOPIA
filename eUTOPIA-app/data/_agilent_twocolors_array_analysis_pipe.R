0## suppose to have one file for each slot, 
## for a two-color experiment we have two samples associated with one slot
agilent.twocolors.analysis <- function(pdata.file, pd.file, type.data = "mRNA",
                                       qdist = c(.9), perc = c(50), num.pc = 5,
  ...
  pdata  <- read.table(pdata.file, sep="\t", header=T)
  pd     <- read.table(pd.file, sep="\t", header=T)
  rgList <- read.maimages(files=pdata$FileName, source="agilent.median", verbose = verbose)
  # generate nc.data, c.data and pd
  st <- fix.data(pd, rgList)
  pd <- st$pd
  ...
  mart <- connect.to.biomart()
  if(verbose) cat("Annotating..\n")
  biomart.data <- query.biomart(mart, comb.data)
  ...
  #return(list(data = fdata, biomart.data = biomart.data, vct = vct))
  #return(list(norm.data = norm.data, combat = comb.data, data = fdata, biomart.data = biomart.data, vct = vct))
...
## PRE-PROCESSING
fix.data <- function(pd, rgList, verbose = TRUE) {
  ...
  #which(pd$SampleID
  while(!is.numeric(n1)) {
    if(is.null(samples)) print(pd)
    else print(pd[-samples,])
    n1 <- read.input.number("Indicate a sample to remove <enter 0 to exit>:")
    if(n1 == 0) {
      n2 <- NULL
      break
    }
    if(is.null(samples))  samples <- n1
    else  samples <- c(samples, n1)
    n1 <- NULL
  }
  # removing selected samples from pd and data
  if(length(samples) > 0) {
    pd <- pd[-samples,]
    data <- data[,-samples]
  }
  # defining the rownames for pd
  test.row.names <- which(duplicated(rownames(pd)))
  if(length(test.row.names) == 0) {
    rownames(pd) <- pd[,1]
    pd <- pd[,-1]
  }
  else
    warning("PD.rownames not valid!")
  # drope levels to 0 
  pd <- droplevels.annot(pd)
  if(verbose) {
    cat(" - Checking if the pd and data are correct..\n")
    cat(colnames(data))
    cat("\n")
    cat("\n")
    cat("\n")
  ...
  # turning in factor numeric fields such as "slide"
  id.slide <- which(colnames(pd) == "slide")
  pd[,id.slide] <- as.factor(pd[,id.slide])
  list(nc.data = nc.data, c.data = c.data, pd = pd)
}
...
pre.proc <- function(nc.data, c.data, var.mds, qdist = c(.75, .9), perc = c(75, 50), verbose = TRUE, plot = TRUE, ...) {
  norm.data <- NULL
  ...
  if(length(qdist) != 1 & length(perc) != 1) {
    ...
    if(plot)  boxplot(log2(filt.data[[n1]]), las=2, cex=0.7, main="Before normalization.")
    ...
    if(plot)  boxplot(log2(filt.data), las=2, cex=0.7, main="Before normalization.")
...
filt.by.neg.probes <- function(data, cdata, qdist = .9, perc = 50, verbose = TRUE, ...) {
  ...
  thres.vec <- apply(cdata, 2, function(x) quantile(x, qdist))
...
filt.by.var.dup.probes <- function(data, qdist = .9, perc = 50, verbose = TRUE, plot = TRUE, ...) {
  ...
  for(j in 1:ncol(data)) {
    ...
    thres.vec <- quantile(dist.var, qdist)
...
remove.batch.effects <- function(comb.data, pd, num.pc, vars, inter = "", method = "Combat", plot = TRUE, verbose = TRUE, ...) {
  ...
  while(flag) {
      ...
      id.var <- grep(batch, batches) 
      print(id.var)
      if(length(id.var) > 0) {
        pre.batches <- batches
        batches <- batches[-id.var]
        #list.mod <- list()
        #list.mod[[1]] <- as.factor(vars$var.int)
        #for(i in 1:length(covs))
        #  list.mod[[(i+1)]] <- as.factor(pd[,covs[i]])
        #print("ssss")
        #print(length(list.mod))
        #print(length(batches))
        #if(length(batches) > 0) {
        #  t <- length(list.mod)
        #  for(i in 1:length(batches))
        #    list.mod[[(i+t)]] <- as.factor(pd[,batches[i]])
        #}
        #print(length(list.mod))
        #mod <- do.call(cbind, list.mod)
        #print(mod)
...
get.sva.batch.effects <- function(comb.data, pd, vars, npc = 10, verbose = T) {
  ...
  print(X)
  assoc.pca <- pc.anaylsis(data = comb.data, pd = X, npc = npc)
...
query.biomart <- function(mart, data, verbose = TRUE, ...) {
  ...
  if(interactive()){
    ...
    # check species
    testh <- grep("hsapiens", mart@dataset)
...
diff.gene.expr <- function(data, des, contrasts, annot = NULL, save.file = NULL, plot = TRUE, verbose = TRUE, ...) {
  ...
  cont.it <- TRUE
  while(cont.it) {
    pvalue <- read.input.number(" - indicate the p.value:")
    fcvalue <- read.input.number(" - indicate the fold change:")
    max.ngenes <- read.input.number(" - indicate the max number of genes:")
    methods <- c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none")
    print(data.frame(p.adjust.methods = methods))
    n1 <- read.input.number(" - indicate the p.value adjust method:")
    if(plot & length(contrasts) < 6) {
      vennDiagram(decideTests(fit2, p.value=pvalue, lfc=log2(fcvalue), adjust.method=methods[n1]),
                  cex=c(.8,.8,0.7),
  ...
  ord.methods <- c("logFC", "abs(logFC) * -log(p-values)")
  print(data.frame(ord.methods = ord.methods))
  n2 <- read.input.number(" - indicate the ordering method:")
  test.save <- rep(F, length(list.top.tables))
  for(i in 1:length(list.top.tables)) {
    ...
    if(n2 == 1)
      list.top.tables[[i]] <- list.top.tables[[i]][order(list.top.tables[[i]]$logFC),]
    else if(n2 == 2) {
      score <- abs(list.top.tables[[i]]$logFC) * -log(list.top.tables[[i]]$P.Value)
      list.top.tables[[i]] <- list.top.tables[[i]][order(score, decreasing = TRUE),]
      list.top.tables[[i]]$score <- sort(score, decreasing = TRUE)
    }
  }
  save.dat <- continue.read.inputs(message="Press [y] to export the top tables in excel files.")
  if(!is.null(save.file) & save.dat) {
    if(verbose) cat(" - saving the top tables.. \n", sep="")
    if(!all(test.save))
      save.xls.file(lists.diff.genes = list.top.tables[which(test.save==T)], file.name = save.file) 
    else
      save.xls.file(lists.diff.genes = list.top.tables, file.name = save.file) 
  }
  else if(is.null(save.file) & save.dat) warning("Output file name (excel) not specified.")
  list.top.tables
}
...
plot.heatmap <- function(data, labels, top = 500) {
  ...
  heatmap.2(data[names(hvars),], Rowv=TRUE, 
            ...
            col=redgreen, xlab="sample", ylab="label", 
            labRow = NULL, dendrogram = "column", 
...
featSelDFP <- function(exprData, skipFactor = 3, zeta = 0.5, piVal = 0.9, overlapping = 2) {
  ...
  x11(); plotDiscriminantFuzzyPattern(dfps, overlapping)
...
COLORS <- c("green3","blue","cyan","magenta","yellow","gray","red","orange",
            ...
            "burlywood3","steelblue4","orangered","purple1","khaki1")
