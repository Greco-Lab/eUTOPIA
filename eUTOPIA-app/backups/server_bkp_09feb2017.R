...
shinyServer(
	function(input, output, session){		
		...
		phRows <- reactive(as.numeric(input$pheno_rows_selected))
                ...
                gVars$inputAnn <- eventReactive(input$load_ann_submit, {
			...
			if(sepS=="OTHER"){
				...
				if(sepS=="TAB"){
					...
					sepChar=sepS
                        ...
                        percMisStr <- percMisStr <- paste0(names(percMis), "[", percMis, "%", "]")
                        shinyjs::html("annMaps", paste0("Mappability: ", paste(percMisStr, collapse=", ")))
			return(annDF)
		})
		...
		gVars$inputPh <- eventReactive(input$load_pheno_submit, {
			...
			print(sepChar)
                        ...
                        phTable <- read.csv(phFile$datapath, header=TRUE, sep=sepChar, stringsAsFactors=FALSE, quote="")
			return(phTable)
		})
		...
		output$phenoDT <- DT::renderDataTable({
			...
			gVars$removedSamplesInfo <- NULL
                        ...
                        shinyBS::updateCollapse(session, "bsSidebar", open="LOAD RAW DATA")
                        shinyBS::updateCollapse(session, "bsSidebar", style = list("LOAD PHENOTYPE DATA"="success", "LOAD RAW DATA"="warning"))
                })
		...
		output$filtered <- DT::renderDataTable({
			...
			c.data  <- data[which(rgList$genes$ControlType==-1),]
                        ...
                        shinyBS::updateCollapse(session, "bsSidebar", open="PROBE FILTERING")
                        shinyBS::updateCollapse(session, "bsSidebar", style = list("LOAD RAW DATA"="success", "PROBE FILTERING"="warning"))
		})
		...
		output$numProbesText <- renderText({
			...
			norm.data <- run.norm(filt.data=filt.data, plot=FALSE)
                        ...
                        shinyBS::updateCollapse(session, "bsSidebar", open="BATCH CORRECTION")
                        shinyBS::updateCollapse(session, "bsSidebar", style = list("NORMALIZATION"="success", "BATCH CORRECTION"="warning"))
                })
		...
		observeEvent(input$batch_submit, {
			...
			gVars$agg.data <- agg.data
                        ...
                        shinyBS::updateCollapse(session, "bsSidebar", open="DIFFERENTIAL ANALYSIS")
                        shinyBS::updateCollapse(session, "bsSidebar", style = list("BATCH CORRECTION"="success", "DIFFERENTIAL ANALYSIS"="warning"))
		})
		...
		output$deTable <- DT::renderDataTable({
			...
			content = function(con){
                                deg.list <- gVars$deg.list
                                WriteXLS(deg.list, ExcelFileName=con, col.names=T, AdjWidth=T, BoldHeaderRow=T)
			}
		)
		...
		output$volcanoPlot <- renderPlot({
			...
			filename = function(){
				paste("Microarray_Analysis_Report_", Sys.Date(), '.PDF', sep='')
			},
			content = function(con){
		...
		output$selSep <- renderUI({
			...
			selectInput("sepSAnno", "Select Column Seperator", choices=gVars$sepChoices, selected=gVars$sepChoices[1])
		})
